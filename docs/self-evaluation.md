...
The best configuration `heading_semantic_openai_dense` (NDCG@5 = 0.896, experiment `d702830f`) outperformed the worst `embedding_semantic_mpnet_hybrid` (NDCG@5 = 0.607, experiment `b6596d30`) by +0.289 NDCG@5. I traced the gap to three factors, in order of impact:
...
**Retriever type is secondary.** Dense retrieval slightly edges out hybrid for the best config (0.896 vs 0.870, per experiment `2964c02f`). This surprised me. Hybrid wins on average across all configs (0.7515 vs 0.7176), but the OpenAI embeddings are good enough that BM25's keyword signal doesn't add value at the top of the ranking.
...
I tested across 4 academic papers with varying characteristics:
...
- **Hybrid approach for production:** Index with both. Use Ollama for bulk/batch operations (overnight re-indexing, dev/test), OpenAI for user-facing real-time queries. The factory pattern makes switching a one-line YAML change.
